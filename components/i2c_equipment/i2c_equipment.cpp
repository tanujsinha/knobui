...
{
  ESP_LOGI(TAG,"Starting DRV2605 initialization...");
  if(!drv.begin(i2c_dev_Callback))
  {
    ESP_LOGE(TAG,"drv2605 init failure");
    return;
  }
  ESP_LOGI(TAG,"DRV2605 begin successful");
  drv.selectLibrary(5);
  ESP_LOGI(TAG,"DRV2605 library 5 selected");
  ...
  // Ensure we're using ERM motor mode for maximum strength
  drv.useERM();  // Use Eccentric Rotating Mass motor (stronger than LRA)
  
  ESP_LOGI(TAG,"DRV2605 initialization complete with ERM motor mode");
}

void i2c_drv2605_haptic_feedback(uint8_t effect)
{
  ESP_LOGI(TAG,"DRV2605 haptic feedback - effect %d", effect);
  drv.setWaveform(0, effect);  // play effect
  drv.setWaveform(1, 0);       // end waveform
  drv.run();                   // play the effect!
  ESP_LOGI(TAG,"DRV2605 haptic feedback sent");
}

// Strong haptic feedback using real-time mode for enhanced intensity
void i2c_drv2605_haptic_feedback_strong(uint8_t intensity) {
    // Set to real-time mode for direct intensity control
    drv.setMode(SensorDRV2605::MODE_REALTIME);
    
    // Set the intensity value (0-255, higher = stronger)
    drv.setRealtimeValue(intensity);
    
    // Hold the vibration for a longer duration to make it more noticeable
    vTaskDelay(pdMS_TO_TICKS(100));
    
    // Stop the vibration by setting to 0
