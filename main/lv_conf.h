 ...
 *If LV_USE_ASSERT_NULL is enabled `LV_ASSERT_NULL` will be performed
 *when the parameter is NULL. (Very fast, recommended)*/
#define LV_USE_ASSERT_NULL          1

/*Check if the parameter is valid (has valid range, does not repeat, etc.)
 *If LV_USE_ASSERT_MEM_INTEGRITY is enabled LVGL will run integrity checks on internal data structures.
 *(Very slow, use it only during development)*/
#define LV_USE_ASSERT_MEM_INTEGRITY 0

/*Check the integrity of `lv_mem` after critical operations. (Quite fast, recommended)*/
